# Dokumentasi Instalasi & Konfigurasi ERPNext v16

Repositori ini memuat panduan lengkap mengenai proses instalasi server, konfigurasi modul bisnis, integrasi sistem, hingga manajemen hak akses pada aplikasi ERPNext menggunakan Docker.

---

### 🚀 Tahap 1: Deployment & Instalasi
Panduan inisialisasi server, konfigurasi `docker-compose.yml`, dan proses deployment kontainer ERPNext.
* [📄 1. Instalasi ERPNext on Docker](tutorial%20setup/1.Instalasi%20ERPNext%20on%20Docker.md)

### 📂 Tahap 2: Konfigurasi Aplikasi
Panduan setup email, integrasi Keycloak, dan konfigurasi penamaan seri transaksi.
* [📄 1. Integrasi Keycloak](tutorial%20setup/2.Integrasi%20Keycloak.md)
* [📄 2. Setup Email Default](tutorial%20setup/3.Setup%20Email%20Default.md)
* [📄 3. Setup Permission Account](tutorial%20setup/4.Setup%20permission%20account.md)


### 🔒 Tahap 3: Konfigurasi Fitur
Panduan manajemen izin akses pengguna, pengaturan hak akses akun, dan kustomisasi format cetakan (Print).
* [📄 1. Print Setup](tutorial%20setup/5.Print%20Setup.md)
* [📄 2. Setup Series for Transaction](tutorial%20setup/6.Setup%20series%20for%20transaction.md)


### 📚 Implementasi Backup Volume
Panduan untuk backup volume
* [📄 1. Backup Volume](tutorial%20setup/7.Backup.md)
* [📄 2. Implementasi Backup](tutorial%20setup/8.Implementasi%20backup.md)
---
### 📚 Sumber Referensi
Kumpulan artikel dan sumber pembelajaran untuk pengembangan ERPNext lebih lanjut.
* [📄 Sumber Artikel Pembelajaran](tutorial%20setup/9.Sumber%20artikel%20pembelajaran.md)